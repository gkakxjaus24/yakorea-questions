/**
 * 베트남어(vi) intent 답변 + 질문 변형 시드 스크립트
 *
 * 동작 방식:
 *   1. DB에서 language='en' 답변 전체 조회
 *   2. Claude Haiku로 일괄 번역 (베트남어)
 *   3. intent_answers에 vi 언어로 upsert
 *   4. DB에서 language='en' 질문 변형 조회 (intent당 최대 8개)
 *   5. Claude Haiku로 일괄 번역
 *   6. intent_questions에 vi 언어로 삽입 (기존 vi 삭제 후 재삽입)
 *
 * 실행: node src/scripts/add_vietnamese_questions_answers.js
 */
...
const TARGET_LANG_NAME = 'Vietnamese (Tiếng Việt, Latin script with diacritics)';
